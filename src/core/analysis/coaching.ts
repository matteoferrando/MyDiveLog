...
type Rule = (agg: Aggregates, dives: Dive[]) => Finding | null;

const ruleGasLevel: Rule = (agg) => {
  ...
  return {
    ...
    area: 'gas',
    severity,
    headline: `Consumo di superficie ${rmv.toFixed(1)} L/min: c'è margine`,
    detail:
...
const ruleGasTrend: Rule = (agg) => {
  const t = agg.rmvTrend;
  if (!t || t.direction !== 'worsening' || t.n < 8) return null;
  return {
    id: 'gas-trend',
    area: 'gas',
    severity: 'warning',
    headline: 'Il consumo sta salendo nel tempo',
    detail:
    ...
    evidence: [
      `Da ${t.firstHalf.toFixed(1)} a ${t.secondHalf.toFixed(1)} L/min fra prima e seconda metà (${t.n} immersioni).`,
      `Variazione stimata ${signed(t.slopePerYear)} L/min all'anno.`,
    ],
...
const ruleBuoyancy: Rule = (agg) => {
  const n = agg.trim.length;
  if (n < BENCHMARK.minBasis || agg.avgTrim === undefined) return null;
  const trim = agg.avgTrim;
  const evidence = [
    `${trim.toFixed(1)} m verticali "sprecati" al minuto mentre tieni la quota, media su ${n} immersioni.`,
    agg.trimTrend
      ? `Tendenza: ${agg.trimTrend.firstHalf.toFixed(1)} → ${agg.trimTrend.secondHalf.toFixed(1)} m/min.`
      : '',
  ].filter(Boolean);
  ...
  if (trim <= BENCHMARK.trimGood) {
    return {
      ...
      area: 'buoyancy',
      severity: 'good',
      headline: `Assetto solido: ${trim.toFixed(1)} m/min di oscillazione`,
      detail:
      ...
      evidence,
      drills: ['Mantieni il livello aggiungendo un compito: bobina, dSMB, gestione stage.'],
      priority: 12,
      basis: n,
    };
  }
  ...
  return {
    ...
    area: 'buoyancy',
    severity,
    headline: `Oscillazione verticale di ${trim.toFixed(1)} m/min a quota tenuta`,
    detail:
    ...
    evidence,
    target: `Scendere sotto ${BENCHMARK.trimGood} m/min di oscillazione media.`,
    drills: [
...
const ruleAscentRate: Rule = (agg, dives) => {
  ...
  return {
    ...
    area: 'ascent',
    severity,
    headline: `Risalite oltre il limite nel ${pct(rate)} delle immersioni`,
    detail:
      `Il limite di riferimento è ${LIMITS.ascentRateDeepMpm} m/min sotto i 10 m e ${LIMITS.ascentRateShallowMpm} m/min sopra. ` +
    ...
    evidence: [
      `${offenders.length} immersioni su ${withProfile.length} con almeno 30 s sopra il limite.`,
      `Di queste, ${shallow} con violazioni sopra i 10 m.`,
      `Picco registrato: ${Math.max(...withProfile.map((d) => d.metrics?.maxAscentRateMpm ?? 0)).toFixed(0)} m/min.`,
    ],
    target: `Portare le immersioni con violazioni sotto il ${pct(BENCHMARK.fastAscentRate)}.`,
    drills: [
...
const ruleSafetyStop: Rule = (agg) => {
  ...
  return {
    ...
    headline: `Sosta di sicurezza completata nel ${pct(rate)} delle immersioni`,
    detail:
      `Consideriamo completata una sosta di almeno ${Math.round(LIMITS.safetyStopMinS / 60)} minuti fra 3 e 6 m. ` +
    ...
    evidence: [
      `${Math.round(rate * agg.safetyStopEligible)} soste complete su ${agg.safetyStopEligible} immersioni valutabili.`,
      'Valutate solo le immersioni in curva oltre i 10 m con profilo campionato.',
    ],
    target: `Superare il ${pct(BENCHMARK.safetyStopRate)} nelle prossime 15 immersioni.`,
    drills: [
...
const ruleCeilingViolations: Rule = (agg, dives) => {
...
const ruleGasReserve: Rule = (agg, dives) => {
...
const ruleCurrency: Rule = (agg) => {
  ...
  if (days > BENCHMARK.layoffDays) {
    return {
      ...
      headline: `${days} giorni dall'ultima immersione`,
      detail:
      ...
      evidence: [
        `Ultima immersione: ${agg.lastDive ? formatDate(agg.lastDive) : '—'}.`,
        `${agg.divesLast12m} immersioni ${periodo} (${perMonth}/mese).`,
      ],
  ...
  if (perMonth < BENCHMARK.perMonth) {
    return {
      ...
      area: 'experience',
      severity: 'warning',
      headline: `${perMonth} immersioni al mese: poche per consolidare`,
      detail:
      ...
      evidence: [`${agg.divesLast12m} immersioni ${periodo}.`, `${agg.divesLast90d} negli ultimi 90 giorni.`],
...
const ruleDataQuality: Rule = (agg, dives) => {
  if (agg.count < 5) return null;
  ...
  const evidence = [
    `${agg.withProfile} immersioni su ${agg.count} hanno un profilo campionato.`,
    `${agg.count - noGas} immersioni su ${agg.count} permettono di calcolare il consumo.`,
  ];
  if (missingVolume > 0) {
...
const ruleDecoExposure: Rule = (agg, dives) => {
  ...
  return {
    ...
    drills: ['Soste con compito in mano: bobina, dSMB, cambio gas simulato.'],
    priority: 14,
...
const ruleGf99: Rule = (agg, dives) => {
  ...
  if (median <= 65 && high > 0) {
    return {
      ...
      target: "Riportare anche i casi isolati sotto l'85% del gradient factor impostato.",
      priority: 30,
      basis: n,
    };
  }
 ...
 */
const ruleFinalAscent: Rule = (agg) => {
  ...
  if (median <= LIMITS.ascentRateShallowMpm && fast === 0) {
    return {
      ...
      area: 'ascent',
      severity: 'good',
      headline: `Ultimi metri controllati: ${median.toFixed(0)} m/min di mediana`,
      detail:
  ...
  return {
    ...
    headline: `Gli ultimi metri li fai a ${median.toFixed(0)} m/min`,
    detail:
    ...
    evidence,
    target: `Ultimi metri sotto i ${LIMITS.ascentRateShallowMpm} m/min: dalla sosta alla superficie ci vuole quasi un minuto.`,
    drills: [
...
const ruleOxygen: Rule = (agg) => {
  ...
  const evidence = [
    `Giornata peggiore per il CNS: ${cns}% il ${o.worstCnsDay.date} su ${o.worstCnsDay.dives} immersioni, limite 100%.`,
    `Giornata peggiore per le OTU: ${otu}, su una dose di riferimento di 300 al giorno quando si fanno più giorni di fila.`,
    `Calcolato dall'app sul profilo con le tabelle NOAA, su ${o.eligible} immersioni. Il computer usa un modello suo e può dare numeri diversi.`,
  ];
 ...
 */
const ruleProfileShape: Rule = (agg) => {
  ...
  const evidence = [
    `Ridiscese dopo essere già risalito: ${median.toFixed(0)} metri l'ora di mediana, ${worst.toFixed(0)} nel caso peggiore, su ${n} immersioni.`,
    `Parte profonda per prima in ${agg.deepestFirstDives} immersioni su ${agg.deepestFirstEligible}${
      trendM !== undefined
        ? `, con la prima metà mediamente ${Math.abs(trendM).toFixed(1)} m ${trendM >= 0 ? 'più profonda' : 'più alta'} della seconda`
        : ''
    }.`,
    ref
      ? `Il tuo quarto peggiore comincia a ${ref.p75.toFixed(0)} m/h${outliers > 0 ? `, e ${outliers} immersioni stanno oltre il doppio di quella soglia` : ''}.`
  ...
  return {
    ...
    headline:
      firstRate < 0.8
        ? `La parte profonda non viene per prima in ${agg.deepestFirstEligible - agg.deepestFirstDives} immersioni su ${agg.deepestFirstEligible}`
        : `${outliers} ${outliers === 1 ? 'immersione si stacca' : 'immersioni si staccano'} dalle tue per ridiscese`,
    detail:
...
const ruleRepetitive: Rule = (agg) => {
  ...
  const evidence = [
    `${n} immersioni dell'archivio sono ripetitive: sono cominciate con dell'azoto ancora in circolo.`,
    `Il carico residuo costa in mediana ${median.toFixed(1)} punti di GF99 all'uscita.`,
    worst
      ? `Il caso peggiore è ${worst.dive.startTime.slice(0, 10)}${worst.surfaceIntervalMin !== undefined ? ` dopo ${worst.surfaceIntervalMin} minuti di superficie` : ''}: ${worst.points.toFixed(1)} punti in più di quanti ne avresti avuti partendo da tessuti puliti.`
      : '',
    agg.surfaceIntervalMedian !== undefined
      ? `Intervallo di superficie mediano: ${agg.surfaceIntervalMedian} minuti.`
      : '',
  ...
  if (median < 2 && (worst?.points ?? 0) < 5) {
    return {
      ...
      area: 'deco',
      severity: 'good',
      headline: `Le tue ripetitive costano poco: ${median.toFixed(1)} punti di GF99`,
      detail:
  ...
  return {
    ...
    severity: 'warning',
    headline: `Le ripetitive escono ${median.toFixed(1)} punti di GF99 più alte`,
    detail:
    ...
    evidence,
    target: 'Sapere, prima di scendere la seconda volta, con quanto margine in meno stai partendo.',
    drills: [
...
function readinessFor(goal: Goal, agg: Aggregates): Readiness {
  ...
  if (goal.id === 'tec') {
    items.push(
      ...
      {
        label: 'Soste di sicurezza completate',
      ...
      {
        label: 'Immersioni con soste decompressive',
        have: agg.decoDives,
        need: 5,
        unit: '',
        met: agg.decoDives >= 5,
...
export function debriefDive(dive: Dive): Observation[] {
  ...
  if (m.ceilingViolationS > 10) {
    out.push({
      severity: 'critical',
      text: `${formatDuration(m.ceilingViolationS)} sopra il tetto di decompressione (max ${m.maxCeilingM?.toFixed(1) ?? '—'} m).`,
    });
  }
  if (m.fastAscentS >= 30 || m.fastShallowAscentS >= 30) {
    out.push({
      severity: m.fastShallowAscentS >= 60 ? 'serious' : 'warning',
      text: `Risalita oltre il limite per ${formatDuration(m.fastAscentS + m.fastShallowAscentS)}, picco ${m.maxAscentRateMpm?.toFixed(0)} m/min${m.fastShallowAscentS >= 30 ? ' (anche sopra i 10 m)' : ''}.`,
    });
  }
  ...
  if (m.rmvLpm !== undefined) {
    out.push({
      ...
      text:
        `Consumo di superficie ${m.rmvLpm.toFixed(1)} L/min` +
        (m.avgDepth !== undefined ? ` a ${m.avgDepth.toFixed(1)} m di media.` : '.'),
    });
  }
  if (m.maxPpo2 !== undefined && m.maxPpo2 > LIMITS.maxPpo2Bottom) {
    out.push({
      severity: m.maxPpo2 > LIMITS.maxPpo2Deco ? 'critical' : 'warning',
      text: `PPO2 di picco ${m.maxPpo2.toFixed(2)} bar, oltre il limite di fondo di ${LIMITS.maxPpo2Bottom} bar.`,
    });
  }
