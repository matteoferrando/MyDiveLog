...
- **Nessuna cancellazione viene propagata.** Servirebbe un registro delle
  eliminazioni; senza, una cancellazione è locale e la sincronizzazione
  successiva la annulla. È la scelta dichiarata — meglio un'immersione di troppo
  che una perduta — ed è il primo pezzo da aggiungere se servirà.
...
Il token vive nella tabella delle impostazioni dell'archivio locale, inserito
una volta dall'interfaccia. Nel repository non c'è nessuna credenziale.
